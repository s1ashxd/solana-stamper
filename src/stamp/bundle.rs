...
impl StampedBundle {
    ...
    pub fn reconstruct(&self, idx: usize) -> StampedTx {
        ...
        StampedTx { buf, len: self.base_len, sig_off: self.sig_off }
    }
...
impl BundleBuilder<'_> {
    ...
    pub fn sign<S: Signer>(self, primary: &S) -> Result<StampedBundle, StamperError> {
        let Self { template, common, per_provider, blockhash } = self;
        ...
        for computed in template.computed() {
            let pk = (computed.compute)(&resolved, template.payer()).map_err(|source| StamperError::ComputeFailed {
        ...
        for (name, value) in &common {
            let slot = template.slot_table.get(name).ok_or_else(|| StamperError::MissingSlotValue {
                name: name.clone(),
                required_by: Vec::new(),
            })?;
            ...
            for op in &slot.patches {
                match (op.kind, value) {
                    (PatchKind::Pubkey32, SlotValue::Pubkey(b)) => patch_pubkey(&mut base, usize::from(op.offset), b),
                    (PatchKind::Hash32, SlotValue::Hash(b)) => patch_hash(&mut base, usize::from(op.offset), b),
                    (PatchKind::U8, SlotValue::U8(n)) => patch_u8(&mut base, usize::from(op.offset), *n),
                    (PatchKind::U16, SlotValue::U16(n)) => patch_u16(&mut base, usize::from(op.offset), *n),
                    (PatchKind::U32, SlotValue::U32(n)) => patch_u32(&mut base, usize::from(op.offset), *n),
                    (PatchKind::U64, SlotValue::U64(n)) => patch_u64(&mut base, usize::from(op.offset), *n),
                    _ => {
        ...
        for (idx, pv) in per_provider.iter().enumerate() {
            ...
            for slot_name in &template.per_provider_slots {
                ...
                let (name, value) = provided.ok_or_else(|| StamperError::BundleMissingPerProviderSlot {
                    idx,
                    name: slot_name.clone(),
                })?;
                let slot = template.slot_table.get(name.as_str()).ok_or_else(|| {
                    StamperError::MissingSlotValue { name: name.clone(), required_by: Vec::new() }
                })?;
                for op in &slot.patches {
            ...
            diffs.push(ProviderDiff { patches, signature: sig });
        }
